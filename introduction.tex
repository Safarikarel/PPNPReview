...
The apparatus has an overall length of 22~m, a diameter of 15~m, and weighs 14\,000~tonnes.
The central feature of the experiment is a superconducting solenoid
of 6\unit{m} diameter. The large magnetic field of 3.8\unit{T} provided by the solenoid is essential for achieving
...
The silicon tracker measures charged particles in $\abs{\eta}< 2.5$.
For 100\GeVc\ particles, an impact parameter resolution of ${\sim}15\mum$ and a \pt\
resolution of about 1.5\% for 100\GeV [100\GeVc] particles are achieved.
The ECAL consists of 75\,848 lead tungstate crystals covering $\abs{ \eta }< 1.479 $ in the
barrel region (EB) and $1.479 <\abs{ \eta } < 3.0$ in two endcap regions (EE).
A two-plane lead/silicon preshower detector is located  in front of the EE.
The photon energy resolution of the ECAL at  $\ET {\approx} 60$\GeV ranges from
 1.1\% and 2.6\% in the EB region and from and from 2.2\% to 5\% in the
endcaps. The jet resolution of the ECAL/HCAL system can be parametrized as
$\Delta E/E \approx 100\% / \sqrt{E\,[\GeVns]} \oplus 5\%$~\cite{Chatrchyan:2013dga}.
Forward calorimeters extend the rapidity coverage provided barrel and endcap detectors.

In the steel return yoke outside the magnet, detectors are embedded for muon identification.
The muon detectors cover $\abs{\eta}< 2.4$ using three detector technologies:
...
interesting events in a fixed time interval of less than 4\mus.
The High Level Trigger (HLT) processor farm, based on commodity hardware,
decreases the event rate to several 100\unit{Hz} for final data storage.
