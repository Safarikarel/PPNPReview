...
typically have a 50\% higher value of $v_2$.  This behaviour has been explained by the hadronization of
the system via constituent quarks which recombine into baryons with $v_2$(baryon)=$(3/2) v_2$(meson)~\cite{Adare:2006ti,Fries:2003vb}.
\item The deviations from ideal behaviour can be systematically calculated by viscous corrections, and
...
plotted as a function of the transverse density of charged particles at mid-rapidity, estimated by ${\rm d}N_{\rm{ch}}/{\rm d}\eta/S$,
...
and is shown in Fig.~\ref{fig:pas:fc:firstresults}.
...
to $\sqrt{s_{\rm{NN}}}=7.7$ GeV~\cite{Adamczyk:2012ku}),
...
showing the transition from the low-\pT\ behaviour, see Fig.~\ref{fig:pas:fc:scaling} (left),
...
${\rm d}N/{\rm d}\pT$, to match with the lower energy PHOBOS data.
...
Fig.~\ref{fig:pas:fc:scaling} (right), and overlaps the PHOBOS Au--Au and
...
The CMS data shown in Fig.~\ref{fig:pas:fc:methods} illustrates the $\pT$ and centrality dependence of
...
The ALICE data showing identified particles (separated using the ${\rm d}E/{\rm d}x$ measured in the ALICE TPC) for $\pT>3$~GeV
is shown in Fig.~\ref{fig:pas:fc:highpt} (left).  The charged pion data on $v_2(\pT)$ is quantitatively similar to
...
high-level track trigger.  The data, shown in Fig.~\ref{fig:pas:fc:highpt} (right), show that the
...
Results are compared to data from $\sqrt{s_{\rm{NN}}}=200$~GeV
...
However, it is clear that the behaviour of the CMS data is much more suggestive of a boost-invariant central
plateau, while the PHOBOS data did not show similar behaviour.
...
\caption[]{(left) ATLAS data showing $v_n(\pT)$ for different centrality intervals and $|\eta|<2.5$, for $n=2-6$~\cite{ATLAS:2012at}.
%Very little dependence on $\eta$ is observed~\cite{ATLAS:2012at}.
...
correlations in~\cite{Aamodt:2011by}.
...
Fig.~\ref{fig:pas:fc:vn} (left) and (right) show the harmonics $v_2 -
...
angular power spectra at different resolution scales.  Reproduced from~\cite{ATLAS:2012at}.
}
...
Another representation of this data is given in Fig.~\ref{fig:pas:fc:powerspec}, which shows the ``angular power spectrum'',
...
events is shown in Fig.~\ref{fig:pas:fc:ucc} (left and middle).
The results shown in Fig.~\ref{fig:pas:fc:ucc} (right)
...
and ATLAS data.
As seen in Fig.~\ref{fig:pas:fc:v1b}(c) the negative slope is similar to what was found by STAR at
...
\begin{figure}[!tb]
\begin{center}
\includegraphics[width=0.98\textwidth]{flowcorrelations_figs/atlas_vnfluc_fig_10.pdf}
\caption[]{
ATLAS data on the event-wise distributions of the harmonic coefficients $v_2 $ -- $v_4$ presented in a selection of
centrality intervals.  Reproduced from~\cite{Aad:2013xma}.
}
...
technique~\cite{Aad:2013xma}.  The distributions $P(v_n)$ are shown in Fig.~\ref{fig:pas:fc:flowfluc1} for $v_2$, $v_3$ and $v_4$
...
projected along the radial direction (a result first given in~\cite{Broniowski:2007ft}).
Figure~\ref{fig:pas:fc:flowfluc2} (right) shows ALICE data~\cite{Abelev:2012di} on $\sigma/\langle v_2 \rangle$ derived
