...
nuclear modifications with increasing $Q^2$.  As shown in Fig.~\ref{fig:pas:salgado},
the large magnitude
of shadowing expected at low $Q^2$ (i.e. low \pT) is reduced substantially even by
$Q^2=100$~GeV$^2$.
Thus, the large values of $M_{\mathrm Z}$ and $M_{\mathrm W}$ generally lead
to rather modest nuclear modifications.
...
Reproduced from~\cite{Salgado:2011wc}.
}
...
Together these give a relatively complete first look at the behaviour of heavy vector bosons in
...
and the CMS muon spectrometer, which also
covers $|\eta|<2.4$ using a variety of gaseous detectors
...
As shown in Fig.~\ref{fig:pas:zw_signal} (left) from CMS, the single muon spectrum at high $\pT$ clearly shows a contribution
...
\caption[]{(left) Charge asymmetry for W candidates, from CMS data~\cite{Chatrchyan:2012nt} (right) Rapidity dependence of ${\rm d}N_{\mathrm{Z}}/{\rm d}y$ compared to PYTHIA scaled to the NNLO cross section~\cite{Aad:2012ew}}
...
The data are compared to the same distribution from PYTHIA (version 6.425), scaled to the NNLO total cross section,
...
with unity within the stated uncertainties.  While small effects at the 10--15\% level are not ruled out, they are not required to make sense of the current measurements.
...
is shown in Fig.~\ref{fig:pas:zw_cent} (left),
...
A similar message is found in the ATLAS Z data, shown in Fig.~\ref{fig:pas:zw_cent} (right), which shows the Z yield, scaled by
...
However, photon measurements do not provide a clear mass peak, nor are they associated with
...
$T_{\mathrm{RHIC}}=221 \pm 19 {\mathrm{stat.}} \pm 19 {\mathrm{syst.}}$ MeV for the 0--20\%
...
as well as a minimum-bias (0--100\%) interval.  A similar analysis performed on \pp\ data are
also shown.
In Fig.~\ref{fig:pas:photon} (left),
...
In Fig.~\ref{fig:pas:photon} (right), the preliminary ATLAS data are shown compared with both
